...
functions {
  // define randon number generator function for truncated Laplace
  real double_exponentialt_rng (real mu, real sig, real lb, real ub) {
    real rt;
    int k = 0;
    while (k == 0) {
      rt = double_exponential_rng(mu, sig);
...
model {
  ...
  for(i in 1:Nc){
    xc_sc[i] ~ double_exponential(xstar_sc[i], nu) T[xl_sc, xu_sc];
  }
...
generated quantities {
  ...
  for(j in 1:Nc) {
    ...
    xc_sc_rep[j] = double_exponentialt_rng(xstar_sc[j], nu, xl_sc, xu_sc);
